...
const Form = (props) => {
  ...
  useEffect(() => {
    if (phases.length === 0) dispatch(getAllPhases(""));
  }, [dispatch]);
  ...
  const phase = phases.find((el) => el._id === id);
  ...
  function onSubmit(values, { setSubmitting }) {
    //CREATE
    if (!id) {
      dispatch(insertPhase(values)).then(() => {
        // dispatch(getAllPhases(""));
        history.push("/contents/phases");
      });
      setAlertMessage("Phase Created Successfully");
      setAlertSeverity("success");
    } else {
      //UPDATE
      const validValues = validateObj(phase, values);
      if (Object.keys(validValues).length === 0) {
        setAlertMessage("Nothing To Update");
        setAlertSeverity("error");
        return;
      } else {
        dispatch(updateOnePhase({ id, ...validValues })).then(() => {
          dispatch(getAllPhases(""));
          history.push("/contents/phases");
        });
        setAlertMessage("Phase Updated Successfully");
        setAlertSeverity("success");
      }
    }
  }
        ...
        >
          {({
            ...
            <form autoComplete="off" noValidate onSubmit={handleSubmit}>
              <CardHeader subheader="Phase Form" />
              <Divider />
              <CardContent>
